...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        self.distance = self.distance + math.sqrt(((firstx - self.x) ** 2) + ((firsty - self.y) ** 2))
    ...
    def get_distance_from(self, object):
        side1 = self.x - object.x
        side2 = self.y - object.y
        distance = math.sqrt((side1 ** 2) + (side2 ** 2))
        return distance

    def get_distance_from_start(self):
        distance = math.sqrt(((self.initialx - self.x) ** 2) + ((self.initialy - self.y) ** 2))
        return distance
    ...
    def closer_to(self, object1, object2):
        distance1 = math.sqrt(((self.x - object1.x) ** 2) + ((self.y - object1.y) ** 2))
        distance2 = math.sqrt(((self.x - object2.x) ** 2) + ((self.y - object2.y) ** 2))
    ...
    def halfway_to(self, object):
        midx = -1 * ((self.x - object.x) / 2)
        midy = -1 * ((self.y - object.y) / 2)
        return Point(self.x + midx, self.y + midy)
